...
function M.attach(s)
	...
	local function toggle()
		...
		if s.selected[item] then s.selected[item] = nil else s.selected[item] = true end
		s.render()
	end
	...
	map(k.multiselect.confirm, function() s.close(true, s.selected) end)
	map(k.multiselect.cancel,  function() s.close(false, nil) end)
