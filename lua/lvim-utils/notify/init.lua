...
local _panel_meta = {
	[levels.TRACE] = { icon_key = "trace", hl = "LvimNotifyDebug",  header_hl = "LvimNotifyHeaderDebug",  sep_hl = "LvimNotifySepDebug",  title_hl = "LvimNotifyTitleDebug"  },
	[levels.DEBUG] = { icon_key = "debug", hl = "LvimNotifyDebug",  header_hl = "LvimNotifyHeaderDebug",  sep_hl = "LvimNotifySepDebug",  title_hl = "LvimNotifyTitleDebug"  },
	[levels.INFO]  = { icon_key = "info",  hl = "LvimNotifyInfo",   header_hl = "LvimNotifyHeaderInfo",   sep_hl = "LvimNotifySepInfo",   title_hl = "LvimNotifyTitleInfo"   },
	[levels.WARN]  = { icon_key = "warn",  hl = "LvimNotifyWarn",   header_hl = "LvimNotifyHeaderWarn",   sep_hl = "LvimNotifySepWarn",   title_hl = "LvimNotifyTitleWarn"   },
	[levels.ERROR] = { icon_key = "error", hl = "LvimNotifyError",  header_hl = "LvimNotifyHeaderError",  sep_hl = "LvimNotifySepError",  title_hl = "LvimNotifyTitleError"  },
}
...
local function _close_panel(level)
	...
	if api.nvim_win_is_valid(p.win) then api.nvim_win_close(p.win, true) end
	if api.nvim_buf_is_valid(p.buf) then api.nvim_buf_delete(p.buf, { force = true }) end
	_panels[level] = nil
end
...
local function _rebuild_panel(level, win_w)
	local p = _panels[level]
	if not p or #p.entries == 0 then return end
	...
	if count > 1 then name = name .. "s" end

	local hdr  = pad_s .. icon .. " " .. name
	local fill = win_w - dw(hdr)
	if fill > 0 then hdr = hdr .. string.rep(" ", fill) end
...
local function _render_prog_channel(id, win_w)
	...
	if not ch.lines or #ch.lines == 0 then
		...
		ch.win = nil; ch.buf = nil; ch.height = nil
		return
	end
	...
	if hdr_fill > 0 then hdr_text = hdr_text .. string.rep(" ", hdr_fill) end
...
local function _show_toast(msg, level, opts)
	...
	local inner_w = 0
	if title then inner_w = math.max(inner_w, dw(title)) end
	for _, l in ipairs(msg_lines) do inner_w = math.max(inner_w, dw(l)) end
	...
	local function push(str, ...)
		...
		for _, m in ipairs({ ... }) do table.insert(marks, { ri, m[1], m[2], m[3] }) end
		ri = ri + 1
	end
...
local function _append_history(msg, level, opts)
	...
	while #_history > max do table.remove(_history, 1) end
end
	...
	for _, p in ipairs(_printers) do
		...
		if not ok then
			vim.schedule(function()
				api.nvim_echo({ { "[lvim-utils.notify] printer '" .. p.name .. "' error: " .. tostring(err), "ErrorMsg" } }, true, {})
			end)
		end
	end
end
...
function M.remove_printer(name)
	for i, p in ipairs(_printers) do
		if p.name == name then table.remove(_printers, i); return end
	end
end
...
function M.notify(msg, level, opts)  _dispatch(msg, level, opts) end
function M.get_history()             return vim.deepcopy(_history) end
function M.clear()                   _history = {} end
...
function M.progress_register(id, opts)
	...
	local ch = _prog_channels[id]
	if opts.name      ~= nil then ch.name      = opts.name      end
	if opts.icon      ~= nil then ch.icon      = opts.icon      end
	if opts.header_hl ~= nil then ch.header_hl = opts.header_hl end
end
...
function M.register_panel(key, opts)
	...
	for i, k in ipairs(PANEL_ORDER) do
		if k == key then table.remove(PANEL_ORDER, i); break end
	end
...
function M.progress_update(id, lines, marks)
	...
	local max_w = _cfg.max_width or 60
	local nw    = min_w
	for _, l in ipairs(lines) do nw = math.max(nw, dw(l)) end
...
function M.progress_clear(id)
	local ch = _prog_channels[id]
	if not ch then return end
	ch.lines = nil; ch.marks = nil; ch.natural_w = nil
	_rebuild_all()
end
...
function M.setup(user_cfg)
	...
	if user_cfg.printers then
		...
		for _, p in ipairs(user_cfg.printers) do
			if     p == "toast"   then M.add_printer("toast",   _show_toast)
			elseif p == "history" then M.add_printer("history", _append_history)
			elseif type(p) == "function"       then M.add_printer(tostring(p), p)
			elseif type(p) == "table" and p.fn then M.add_printer(p.name or tostring(p), p.fn)
			end
		end
		if not M.has_printer("history") then M.add_printer("history", _append_history) end
	end
...
do
	...
	for name, opts in pairs(colors) do
		if name:match("^LvimNotify") then nc[name] = opts end
	end
	hl.register(nc, true)
end
