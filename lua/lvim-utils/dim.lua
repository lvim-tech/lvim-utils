...
--
-- Two consumers share this: lvim-colorscheme's focus-follow `dim_inactive`, and lvim-ui's surface backdrop
-- "dim" mode (dim the windows behind a float instead of covering them). This module owns only the namespace +
-- the blend; WHICH windows to dim, WHEN, and restore is each consumer's own (they may overlap — the namespace
-- is identical, and a stale reset self-heals on the next reconcile).
--
...
--- (Re)build the dim namespace from the CURRENT global highlights, muting each group's fg/sp toward `bg_hex`
...
---@return integer ns
function M.darken(dark_hex, amount)
    ...
    for name, def in pairs(api.nvim_get_hl(0, {})) do
        if is_preserved(name) then
            api.nvim_set_hl(M.darken_ns, name, def)
        elseif def.link then
            api.nvim_set_hl(M.darken_ns, name, { link = def.link })
        else
            if def.fg then
